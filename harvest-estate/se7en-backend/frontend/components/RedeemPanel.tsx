    ...
    <div className="rounded-2xl border border-white/10 bg-black/50 p-6 shadow-lg">
      <div className="mb-4 flex items-center justify-between text-xs uppercase tracking-wide text-gray-400">
        <div>
          ...
          <p className="text-xs text-gray-400">Burns HRVST, debits Treasury stables, and records a CPA journal entry.</p>
        </div>
        <div className="flex items-center gap-2">
          <span className={`rounded-full px-3 py-1 ${currentMode === 'demo' ? 'bg-violet-600/60 text-white' : 'bg-white/10 text-gray-300'}`}>
            Demo
          </span>
          <span className={`rounded-full px-3 py-1 ${currentMode === 'live' ? 'bg-amber-500/60 text-black' : 'bg-white/10 text-gray-300'}`}>
            Live
          </span>
          <button
            className="rounded-lg border border-white/20 px-3 py-1 text-[11px] uppercase text-gray-200 hover:border-white"
            type="button"
            onClick={() => onModeChange(currentMode === 'demo' ? 'live' : 'demo')}
          >
            Toggle
          </button>
        </div>
